# Critique: Behavioral Test Coverage
...
- **DB fixture design is accurate.** The in-memory SQLite schemas match the actual SQL queries in the code. `insertMessage` and `insertPart` factories produce the right JSON structure.
- **SSE helper is well-designed.** `collectSSEEvents` handles both count-based and timeout-based resolution, which addresses the timing flakiness risk the plan calls out.
- **Risks section is honest.** Calling out offline/reconnect complexity, SSE timing concerns, and schema coupling is exactly what a good plan should do.
- **`checkNeedsAttention` test coverage is thorough.** Both positive and negative cases, including the tricky "let me know" without question mark and "should I" in non-question context.
- **The plan explicitly avoids testing internals.** No tests for `updateAgentState`, `addFeedItem`, etc. directly — only through their HTTP effects. This gives maximum refactoring freedom.
...
|---|---------|----------|--------|
| 1 | `initState()` not called in `createApp()` — all tests crash | 🔴 Critical | Add 1 line + update plan step |
| 2 | `pollOpenCodeActivity` not returned — Task 7 blocked | 🔴 Critical | Add to return object |
| 3 | `test-server.js` missing from Files to Create table | 🟡 Important | Add 1 row |
| 4 | No SSE test teardown — `npm test` hangs | 🟡 Important | Add cleanup function or afterEach |
| 5 | `state=attention` substatus clearing not tested | 🟡 Important | Add 1 test case to Task 3 |
| 6 | Constants needlessly inside `createApp()` closure | 🟡 Important | Clarify module-level exports |
| 7 | No feed item `type` validation test | 🟡 Important | Add 1 test case to Task 2 |
| 8 | Temp directory cleanup | 🟢 Minor | Return cleanup function |
| 9 | No concurrent SSE client test | 🟢 Minor | Add 1 test to Task 6 |
| 10 | No `engines` field for Node version guard | 🟢 Minor | Add 1 field to package.json |
